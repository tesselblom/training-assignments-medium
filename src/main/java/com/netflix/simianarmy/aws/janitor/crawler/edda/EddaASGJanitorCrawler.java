...
public class EddaASGJanitorCrawler implements JanitorCrawler {
	...
	/**
	 * The name representing the additional field name of launch configuration
	 * name.
	 */
	...
	/**
	 * The name representing the additional field name of ASG suspension time
	 * from ELB.
	 */
	public static final String ASG_FIELD_SUSPENSION_TIME = "ASG_SUSPENSION_TIME";

	/**
	 * The name representing the additional field name of ASG's last
	 * change/activity time.
	 */
	public static final String ASG_FIELD_LAST_CHANGE_TIME = "ASG_LAST_CHANGE_TIME";

	/**
	 * The regular expression patter below is for the termination reason added
	 * by AWS when an ASG is suspended from ELB's traffic.
	 */
	...
	private Resource parseJsonElementToresource(String region, JsonNode jsonNode,
		...
		Resource resource = new AWSResource().withId(asgName).withRegion(region).withResourceType(AWSResourceType.ASG)
		...
		addsInstancesAndELBs(resource, jsonNode, lcNameToCreationTime);

		setFieldTimeASGSuspendedFromELB(asgName, region, resource, jsonNode);

		return resource;

	}

	private void setFieldTimeASGSuspendedFromELB(String asgName, String region, Resource resource, JsonNode jsonNode) {
	...
	private void addsInstancesAndELBs(Resource resource, JsonNode jsonNode, Map<String, Long> lcNameToCreationTime) {
	...
	private Map<String, Long> getLaunchConfigCreationTimes(String region) {
		...
		try {
			...
			LOGGER.error(
					String.format("Failed to get Jason node from edda for lc creation times in region %s.", region), e);
		}
	...
	private void refreshAsgLastChangeTime() {
		...
		for (String region : regions) {
			...
			try {
				...
				LOGGER.error(String.format("Failed to get Jason node from edda for ASG last change time in region %s.",
						region), e);
			}
			...
			for (Iterator<JsonNode> it = jsonNode.getElements(); it.hasNext();) {
				...
				LOGGER.debug(
						String.format("The last change time of ASG %s is %s", asgName, new DateTime(lastChangeTime)));
